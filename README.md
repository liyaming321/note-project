...
People Wiki 是一个运行在个人电脑上的本地知识库系统，用来管理个人日记、Markdown 笔记、知识记录、代码片段、浏览器书签和图片资源。系统采用 Spring Boot + Vue 3 构建，数据默认持久化到用户目录下的 H2 文件数据库，并通过 Apache Lucene 提供本地全文检索能力，向量语义检索和混合检索模式。

项目目标是“开箱即用、离线可用、可备份恢复”：开发阶段可以前后端分离运行，部署阶段可以打成单一可执行 Jar，启动后直接访问浏览器页面。

本项目纯粹服务于个人记录日记、笔记和知识沉淀，不追求团队协作、复杂权限或企业级流程。一切设计和功能都优先“方便、简洁、低打扰”：能少一步就少一步，能本地完成就不依赖外部服务，智能能力只是辅助整理和检索，不应让记录本身变复杂。

## 功能概览

- 笔记管理：创建、编辑、详情查看、逻辑删除、恢复、永久删除。
- Markdown 编辑：Vditor 编辑器、Markdown 预览、代码高亮、图片粘贴上传。
- 用途：支持管理“日记、灵感、学习、项目、资料、代码片段”等个人用途，并按用途筛选笔记；Markdown / 代码 / 普通文本仅表示内容格式。
- 分类与标签：分类树、标签云、分类管理、标签管理、标签筛选。
...
- 搜索体验：搜索历史、常用搜索、清空历史、一键复制搜索条件、搜索反馈和混合搜索权重调优。
- 知识整理：待整理视图集中展示链接导入资料、未分类、无标签、缺摘要和长期未更新笔记，并给出元数据建议。
- LLM 总结：支持阿里百炼和 DeepSeek，为笔记生成摘要、标签和分类建议。
- 版本历史：编辑自动保存历史版本，支持查看与恢复。
- 导入导出：Markdown 导入、书签 HTML 导入、链接导入预览、Markdown / ZIP 导出。
- 备份维护：数据、索引、图片目录一键备份，展示最近备份时间、大小、校验值和目录健康状态，支持启动时恢复备份。

## 页面预览

### 工作台首页

首页采用蓝白色工作台布局，左侧为导航、分类和常用标签，顶部提供搜索、导入导出、备份和设置入口。筛选区默认只保留分类、标签、语言，更多条件可展开；完整标签列表统一进入设置页“标签管理”，避免标签过多挤占侧栏空间。
...
新建笔记页提供标题、用途、内容格式、语言、发布状态、分类、标签、收藏和置顶配置，下方是 Vditor 正文编辑区，支持 Markdown、代码、普通文本、图片粘贴上传和实时预览。

![新建笔记](docs/images/note-editor.png)

### 完整筛选面板

展开后可使用用途、内容格式、搜索范围、排序、发布状态、更新时间范围、置顶、收藏、已删除等高级筛选。
...
设置页“搜索调优”可以调整混合搜索权重，配置会保存到本地数据目录下的 `search-tuning.json`。搜索结果卡片支持标记“有用 / 不相关”，反馈会写入本地 `search-feedback.json`，用于后续复盘召回质量和排序权重。

相关 API：

```text
GET /api/admin/search-tuning
PUT /api/admin/search-tuning
POST /api/search/feedback
GET /api/admin/search-feedback-summary
```
...
请求会先按问题和筛选条件执行混合搜索，召回相关笔记，再调用 LLM 生成回答。响应包含 `answer`、`provider`、`model` 和 `citations`；每个引用包含笔记 ID、标题、命中片段和详情页链接。前端“问知识库”面板支持连续提问、追问上下文、引用筛选、答案重新生成、严格基于引用回答、查看引用并跳转原文。
...
- 搜索调优：调整混合搜索权重，查看搜索反馈总数、有用反馈和不相关反馈。
- 配置中心：检查 LLM、Embedding、数据目录、全文索引目录、向量索引目录和图片目录，支持 LLM Provider 连接测试。
- 标签管理：查看完整标签列表，按标签筛选笔记，删除不再使用的标签；删除标签只会从笔记上移除该标签，不会删除笔记。
- 用途管理：管理个人用途，支持新增、重命名、排序、删除和按用途筛选；删除用途只会清空笔记上的用途关联，不会删除笔记。
- 待整理：展示缺少分类、标签、摘要、链接导入资料和长期未更新笔记，并给出摘要、标签和分类建议。

待整理工作台入口也出现在首页左侧导航，便于集中处理导入草稿和元数据缺失内容。当前合并与拆分先落地为产品方案：合并前保留来源快照，拆分时生成新笔记并保留原笔记历史，危险操作需要二次确认和可撤销窗口。

相关 API：

```text
GET /api/admin/configuration-checklist
POST /api/admin/llm-providers/{provider}/test
DELETE /api/tags/{id}
GET /api/note-kinds
POST /api/note-kinds
PUT /api/note-kinds/{id}
DELETE /api/note-kinds/{id}
GET /api/admin/organize-candidates
POST /api/admin/organize-candidates/apply
GET /api/admin/backup-info
```

前端性能方面，路由页面已改为懒加载，Vditor 编辑器和样式改为进入详情 / 编辑页时动态加载，Vite 构建将 Vditor、Ant Design Icons、highlight.js 和通用依赖拆分为独立 chunk，降低首页入口包体积。
...
每次下载完整备份后，系统会在本地数据目录记录备份文件名、大小、创建时间和 SHA-256 校验值。设置页“备份恢复”面板会展示最近备份信息，并检查数据目录、数据库文件、全文索引、向量索引和图片目录是否可用。
...
第八阶段已完成体验、稳定性与性能优化，包括编辑器深度体验、导入链路批量化、搜索与问答质量调优、知识整理工作流、备份健康与目录检查、前端懒加载拆包、配置中心与本地模型引导、测试文档和发布收口。
